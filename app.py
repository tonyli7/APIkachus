...
def pokemon(tag=""):
    ...
    if tag=="":#if the tag is empty
        ...
        request_move_url = urlopen(move_url)
        move_result = request_move_url.read().decode('utf-8')
        move_r = json.loads(move_result)
        #---------------------------------------
        
        info=move_r['responseData']['results'][random.randrange(len(move_r['responseData']['results']))]['url']
        #-----------------making list of img urls------------------------
       # imglist=[]
       # for imgurl in info:
       #     for ext in ['.jpg','.png','.jpeg']:
       #         #checks if its an img based on extension
       #         if ext in ((info[imgurl]).lower()).encode('utf-8').decode('utf-8').strip():
       #             imglist+=[((info[imgurl])).encode('utf-8').decode('utf-8').strip()]
        #----------------------------------------------------------------
